"""The Kent distribution over vectors on the sphere."""
...
class Kent(distribution.AutoCompositeTensorDistribution):
  ...
  * `concentration = kappa`; scalar real >= 0, concentration of samples around
  ...
  def __init__(self,
    ...
    with tf.name_scope(name) as name:
      ...
      self._concentration = tensor_util.convert_nonref_to_tensor(
          concentration, name='angle', dtype=dtype)
  ...
  def majoraxes(self):
    """Concentration parameter."""
    return self._majoraxes

  @property
  def minoraxes(self):
    """Concentration parameter."""
    return self._minoraxes
  ...
  def _log_unnormalized_prob(self, samples, concentration=None, ellipticity=None):
    if concentration is None:
      concentration = tf.convert_to_tensor(self.concentration)
    if ellipticity is None:
      ellipticity = tf.convert_to_tensor(self.ellipticity)

    bcast_mean_dir = (self.mean_direction +
                      tf.zeros_like(concentration)[..., tf.newaxis])
    bcast_major_ax = (self.major_axes +
                      tf.zeros_like(concentration)[..., tf.newaxis])
    bcast_minor_ax = (self.minor_axes +
                      tf.zeros_like(concentration)[..., tf.newaxis])

    inner_product_g1 = tf.reduce_sum(samples * bcast_mean_dir + , axis=-1)
    inner_product_g2 = tf.reduce_sum(samples * bcast_major_ax + , axis=-1)**2
    inner_product_g3 = tf.reduce_sum(samples * bcast_minor_ax + , axis=-1)**2

    return concentration * inner_product_g1 + ellipticity*(inner_product_g2 + inner_product_g3)

  def _log_normalization(self, concentration=None, ellipticity=None):
    ...
    safe_ellip = tf.where(ellipticity > 0, ellipticity,
                          tf.ones_like(ellipticity))
