...
class CVAE(tf.keras.Model):
    ...
    def __init__(self, params, bounds, masks, verbose = False):
        ...
        all_input_yf = tf.keras.Input(shape=(int(self.y_dim/2 + 1), self.n_channels))
        ...
        conv = tf.keras.layers.Conv1D(filters=64, kernel_size=64, strides=1, activity_regularizer=regularizers.l2(1e-5), padding='same')(all_input_y)
        ...
        conv = tf.keras.layers.Conv1D(filters=64, kernel_size=32, strides=1,activity_regularizer=regularizers.l2(1e-5), padding='same')(conv)
        conv = tf.keras.layers.BatchNormalization()(conv)
        conv = tf.keras.layers.Activation(self.act)(conv)
        conv = tf.keras.layers.MaxPool1D(pool_size=2, strides=2)(conv)
        ...
        conv = tf.keras.layers.Conv1D(filters=32, kernel_size=16, strides=2,activity_regularizer=regularizers.l2(1e-5),padding='same')(conv)
        conv = tf.keras.layers.BatchNormalization()(conv)
        conv = tf.keras.layers.Activation(self.act)(conv)
        conv = tf.keras.layers.MaxPool1D(pool_size=2, strides=2)(conv)
        ...
        conv = tf.keras.layers.Conv1D(filters=16, kernel_size=8, strides=2, activity_regularizer=regularizers.l2(1e-5),padding='same')(conv)
        conv = tf.keras.layers.BatchNormalization()(conv)
        conv = tf.keras.layers.Activation(self.act)(conv)
        conv = tf.keras.layers.MaxPool1D(pool_size=2, strides=2)(conv)
        ...
        fconv = tf.keras.layers.Conv1D(filters=64, kernel_size=32, strides=1, activity_regularizer=regularizers.l2(1e-5),padding='same')(all_input_yf)
        fconv = tf.keras.layers.BatchNormalization()(fconv)
        fconv = tf.keras.layers.Activation(self.act)(fconv)
        fconv = tf.keras.layers.MaxPool1D(pool_size=2, strides=2)(fconv)

        fconv = tf.keras.layers.Conv1D(filters=32, kernel_size=16, strides=1, activity_regularizer=regularizers.l2(1e-5),padding='same')(fconv)
        fconv = tf.keras.layers.BatchNormalization()(fconv)
        fconv = tf.keras.layers.Activation(self.act)(fconv)
        fconv = tf.keras.layers.MaxPool1D(pool_size=2, strides=2)(fconv)

        fconv = tf.keras.layers.Conv1D(filters=16, kernel_size=8, strides=1, activity_regularizer=regularizers.l2(1e-5),padding='same')(fconv)
        fconv = tf.keras.layers.BatchNormalization()(fconv)
        fconv = tf.keras.layers.Activation(self.act)(fconv)
        fconv = tf.keras.layers.MaxPool1D(pool_size=2, strides=2)(fconv)
        ...
        fconv = tf.keras.layers.Flatten()(fconv)
        
        conv = tf.keras.layers.concatenate([conv, fconv])

        lin_layers = 4096, 2048, 1024
        #lin_layers = 2048, 1024, 512
        #lin_layers = 1024, 512, 256, 256
        #lin_layers = 1024, 512, 256
        ...
        self.encoder_r1 = tf.keras.Model(inputs=[all_input_y, all_input_yf], outputs=r1)
        if self.verbose:
        ...
        self.encoder_q = tf.keras.Model(inputs=[all_input_y, all_input_yf, q_input_x], outputs=q)
        if self.verbose:
        ...
        self.decoder_r2 = tf.keras.Model(inputs=[all_input_y, all_input_yf, r2_input_z], outputs=r2)
        if self.verbose:
            print(self.decoder_r2.summary())

    def encode_r1(self, y=None, yf=None):
        mean, logvar, weight = tf.split(self.encoder_r1([y, yf]), num_or_size_splits=[self.z_dim*self.n_modes, self.z_dim*self.n_modes,self.n_modes], axis=1)
    ...
    def encode_q(self, x=None, y=None, yf=None):
        return tf.split(self.encoder_q([y,yf,x]), num_or_size_splits=[self.z_dim,self.z_dim], axis=1)

    def decode_r2(self, y=None, yf=None,z=None, apply_sigmoid=False):
        return tf.split(self.decoder_r2([y,yf,z]), num_or_size_splits=[self.x_dim_nonperiodic, 2*self.x_dim_periodic, self.x_dim_sky + 1, self.x_dim_nonperiodic, self.x_dim_periodic, 1], axis=1)
    ...
    def compute_loss(self, x, y, noiseamp=1.0, ramp = 1.0):
        ...
        yf = tf.transpose(tf.signal.rfft(tf.transpose(y, [0,2,1])), [0,2,1])
        y = tf.cast(y, dtype=tf.float32)
        y = tf.keras.activations.tanh(y)
        x = tf.cast(x, dtype=tf.float32)
        
        mean_r1, logvar_r1, logweight_r1 = self.encode_r1(y=y, yf=yf)
        ...
        mean_q, logvar_q = self.encode_q(x=x,y=y,yf=yf)
        ...
        mean_nonperiodic_r2, mean_periodic_r2, mean_sky_r2, logvar_nonperiodic_r2, logvar_periodic_r2, logvar_sky_r2 = self.decode_r2(z = z_samp, y=y, yf=yf)
    ...
    def gen_samples(self, y, ramp=1.0, nsamples=1000, max_samples=1000):
        ...
        for i in range(samp_iterations):
            ...
            mean_nonperiodic_r2_temp, mean_periodic_r2_temp, mean_sky_r2_temp, logvar_nonperiodic_r2_temp, logvar_periodic_r2_temp, logvar_sky_r2_temp = self.decode_r2(z=z_samp, y=y, yf=yf)
    ...
    def call(self, inputs):
        ...
        yf = tf.transpose(tf.signal.rfft(tf.transpose(inputs, [0,2,1])), [0,2,1])
        mean_r1, logvar_r1, logweight_r1 = self.encode_r1(y=inputs,yf=yf)
        scale_r1 = self.EPS + tf.sqrt(tf.exp(logvar_r1))
        ...
        mean_nonperiodic_r2, mean_periodic_r2, mean_sky_r2, logvar_nonperiodic_r2, logvar_periodic_r2, logvar_sky_r2 = self.decode_r2(z=z_samp,y=inputs, yf=yf)
